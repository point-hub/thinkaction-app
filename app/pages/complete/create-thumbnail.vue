...
const { form, formErrors } = useFormProgressCreate();
...
const createThumbnail = async (blob: Blob, targetWidth = 400) => {
  return new Promise<Blob>((resolve) => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((thumbBlob) => resolve(thumbBlob!), 'image/webp', 0.9);
    };
...
const onNext = async () => {
  ...
  const fullBlob = await toBlob();

  const thumbnailBlob = await createThumbnail(fullBlob as Blob, 256);

  form.value.media_blob = fullBlob;
  form.value.thumbnail_blob = thumbnailBlob;
  form.value.goal_id = route.query.goal_id?.toString() ?? '';
  navigateTo(`/complete/create-caption?goal_id=${route.query.goal_id}`);
};
</script>
  ...
  <layouts-with-aside aside-title="">
    <div class="max-w-lg mx-auto lg:p-8">
      <div class="p-4 bg-white flex flex-col gap-4 border border-gray-200 shadow-lg lg:rounded-xl">
        ...
        <p class="text-sm">Congratulations! So, please add a note to explain what you’ve successfully done. It’s a way to track progress and make sure everything is clear!</p>
        ...
        <div class="flex-1 w-full text-sm space-y-4 overflow-y-auto">
          <div class="flex justify-start space-x-1 w-90%">
            ...
            <div class="bg-slate-100 text-slate-600 rounded-lg p-3">
              <p>Upload your image that represents what you’ve achieved</p>
            </div>
          </div>
